...
def generate_class_pyi_def(
    ...
    outputPYIMethodFile.write(
        f"class {class_name}Proxy({', '.join(itk_class.parent_class)}):\n"  # if
    )
...
def init_class_pyi_template_file(pyiFile: Path, submodule_name: str) -> None:
...
def write_class_proxy_pyi(pyiFile: Path, class_name: str, interfaces_code: str) -> str:
    # Write interface files to the stub directory to support editor autocompletion
    with open(pyiFile, "a+") as pyiFile:
...
if __name__ == "__main__":
    ...
    for invalid_index_file in invalid_index_files:
        os.remove(invalid_index_file)

    for missing_index_file in missing_index_files:
        # continue on without missing file, display warning
        print(
            f"WARNING: index file {missing_index_file} is missing, Python stub hints will not be generated for this "
            f"file. "
        )
        index_files.remove(missing_index_file)

    if len(index_files) == 0:
        raise Exception(f"No index files were found in directory {options.pkl_dir}")
    ...
    pickled_files = list(indexed_pickled_files)

    output_init_import_file = init_init_import_file()
    output_proxy_import_file = init_proxy_import_file()

    class_files = [pickled_files[0]]
    for file in sorted(pickled_files[1:]):
        current_class_name = re.split("\.", PurePath(class_files[0]).parts[-1])[0]
        next_class_name = re.split("\.", PurePath(file).parts[-1])[0]

        if current_class_name == next_class_name:
            class_files.append(file)
        else:
            class_name = unpack(class_files, options.pkl_dir)
            if class_name is not None:
                output_init_import_file.write(f"from .{class_name}Template import *\n")
                output_proxy_import_file.write(f"from .{class_name}Proxy import *\n")
            class_files = [file]
